...
# 注意: 実際のバックアップIDをステップ2の出力から取得してください

# 4. バックアップから復元
# 重要: 以下のコマンドは例です。実際のバックアップIDとプロジェクトIDに置き換えてください
...
**重要**: ロールバック手順を実行する前に、以下の連絡先を必ず記入してください。

ロールバック中に問題が発生した場合の連絡先：

- インフラ担当: ________________（必須）
- アプリケーション担当: ________________（必須）
- データベース担当: ________________（必須）
- 緊急連絡: ________________（必須）
